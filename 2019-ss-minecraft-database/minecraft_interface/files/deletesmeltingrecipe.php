	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deletesmeltingrecipe.php" method="post">
						<b>Delete Smelting Recipe</b>
						<p>Input ID:
							<input type="text" name="sr_iid" size="30" value="" />
						</p>
						<p>Output ID:
							<input type="text" name="sr_oid" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['sr_iid']))){
							$errors[] = 'Missing Input ID';
						} else {
							$id1 = trim($_POST['sr_iid']);
						}
						if(empty(trim($_POST['sr_oid']))){
							$errors[] = 'Missing Output ID';
						} else {
							$id2 = trim($_POST['sr_oid']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "DELETE FROM smelting_recipe WHERE IID = ? AND OID = ?";
							...
							if($affected_rows == 1) {
								echo 'Smelting Recipe Deleted Successfully';
