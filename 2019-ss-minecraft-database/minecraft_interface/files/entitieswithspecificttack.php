	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/entitieswithspecificttack.php" method="post">
						<b>Entities with Specific Attack</b>
						<p>Attack Damage:
							<input type="text" name="a_atkdmg" size="30" value="" />
						</p>
						<p>Greater/Equal/Less:
							<input type="text" name="a_gel" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['a_atkdmg']))){
							$errors[] = 'Missing Attack Damage';
						} else {
							$atkdmg = trim($_POST['a_atkdmg']);
						}
						if(empty(trim($_POST['a_gel']))){
							$errors[] = 'Missing Greater/Equal/Less';
						} else {
							$gel = trim($_POST['a_gel']);
							if($gel != 'Greater' && $gel != 'Equal' && $gel != 'Less'){
								$errors[] = 'Greater/Equal/Less Must be Greater/Equal/Less';
							}
						}
						...
						if(empty($errors)){
							...
							if($gel == 'Greater'){
								$query = "SELECT Name, AtkDmg FROM entity WHERE AtkDmg >  '" . $atkdmg . "' ORDER BY Name ASC";
								...
								if($response){
									...
									while($row = mysqli_fetch_array($response)){
										...
										$row['AtkDmg'] . '</td><td align="left"></tr>';
									}
								...
								$query = "SELECT Name, AtkDmg FROM entity WHERE AtkDmg > 0 AND AtkDmg <  '" . $atkdmg . "' ORDER BY Name ASC";
								$response = @mysqli_query($dbc, $query);
