	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/tooltoharvestblock.php" method="post">
						<b>Tool to Harvest Block</b>
						<p>Block Name:
					...
					if(isset($_POST['submit'])){
						...
						if(empty($errors)){
							...
							$query = "SELECT H.Tool FROM block B, harvest_tool H WHERE B.Name =  '" . $name . "' AND B.ID = H.BID ORDER BY H.Tool ASC";
							$response = @mysqli_query($dbc, $query);
							
							if($response){
								echo '</td><td valign="top"><table align="left" cellspacing="5" cellpadding="8">
										<tr>
											<td align="left"><b>Tool</b></td>
										</tr>';
								while($row = mysqli_fetch_array($response)){
									echo '<tr><td align=align"left">' .
									$row['Tool'] . '</td><td align="left"></tr>';
								}
