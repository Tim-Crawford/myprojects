	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertharvesttool.php" method="post">
						<b>Insert Harvest Tool</b>
						<p>Block ID:
							<input type="text" name="ht_bid" size="30" value="" />
						</p>
						<p>Tool:
							<input type="text" name="ht_tool" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['ht_bid']))){
							$errors[] = 'Missing Block ID';
						} else {
							$id = trim($_POST['ht_bid']);
						}
						if(empty(trim($_POST['ht_tool']))){
							$errors[] = 'Missing Tool';
						} else {
							$tool = trim($_POST['ht_tool']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "INSERT INTO harvest_tool (BID, Tool)
										VALUES (?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ss", $id, $tool);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Harvest Tool Inserted Successfully';
