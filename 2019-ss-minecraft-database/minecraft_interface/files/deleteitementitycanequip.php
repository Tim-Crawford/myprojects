	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deleteitementitycanequip.php" method="post">
						<b>Delete Item Entity Can Equip</b>
						<p>Entity ID:
							<input type="text" name="ce_eid" size="30" value="" />
						</p>
						<p>Item ID:
							<input type="text" name="ce_iid" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['ce_eid']))){
							$errors[] = 'Missing Entity ID';
						} else {
							$id1 = trim($_POST['ce_eid']);
						}
						if(empty(trim($_POST['ce_iid']))){
							$errors[] = 'Missing Item ID';
						} else {
							$id2 = trim($_POST['ce_iid']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "DELETE FROM can_equip WHERE EID = ? AND IID = ?";
							...
							if($affected_rows == 1) {
								echo 'Item Entity Can Equip Deleted Successfully';
