	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deleteitementitybreedswith.php" method="post">
						<b>Delete Item Entity Breeds With</b>
						<p>Entity ID:
							<input type="text" name="bw_eid" size="30" value="" />
						</p>
						<p>Item ID:
							<input type="text" name="bw_iid" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['bw_eid']))){
							$errors[] = 'Missing Entity ID';
						} else {
							$id1 = trim($_POST['bw_eid']);
						}
						if(empty(trim($_POST['bw_iid']))){
							$errors[] = 'Missing Item ID';
						} else {
							$id2 = trim($_POST['bw_iid']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "DELETE FROM breeds_with WHERE EID = ? AND IID = ?";
							...
							if($affected_rows == 1) {
								echo 'Item Entity Breeds With Deleted Successfully';
