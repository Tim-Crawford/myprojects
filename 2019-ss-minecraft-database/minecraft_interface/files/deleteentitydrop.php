	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deleteentitydrop.php" method="post">
						<b>Delete Entity Drop</b>
						<p>Item ID:
					...
					if(isset($_POST['submit'])){
						...
						if(empty(trim($_POST['ed_iid']))){
							...
							$id1 = trim($_POST['ed_iid']);
						}
						if(empty(trim($_POST['ed_eid']))){
							$errors[] = 'Missing Entity ID';
						} else {
							$id2 = trim($_POST['ed_eid']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "DELETE FROM entity_drop WHERE IID = ? AND EID = ?";
							...
							if($affected_rows == 1) {
								echo 'Entity Drop Deleted Successfully';
