	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertcraftingrecipe.php" method="post">
						<b>Insert Crafting Recipe</b>
						<p>ID Input Slot 1:
							<input type="text" name="iid1" size="30" value="" />
						</p>
						<p>ID Input Slot 2:
							<input type="text" name="iid2" size="30" value="" />
						</p>
						<p>ID Input Slot 3:
							<input type="text" name="iid3" size="30" value="" />
						</p>
						<p>ID Input Slot 4:
							<input type="text" name="iid4" size="30" value="" />
						</p>
						<p>ID Input Slot 5:
							<input type="text" name="iid5" size="30" value="" />
						</p>
						<p>ID Input Slot 6:
							<input type="text" name="iid6" size="30" value="" />
						</p>
						<p>ID Input Slot 7:
							<input type="text" name="iid7" size="30" value="" />
						</p>
						<p>ID Input Slot 8:
							<input type="text" name="iid8" size="30" value="" />
						</p>
						<p>ID Input Slot 9:
							<input type="text" name="iid9" size="30" value="" />
						</p>
						<p>ID Output:
							<input type="text" name="oid" size="30" value="" />
						</p>
						<p>Output Amount:
							<input type="text" name="cr_oamnt" size="30" value="" />
						</p>
						<p>Is Shapeless:
							<input type="text" name="cr_shplss" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['iid1']))){
							$id1 = 'Empty';
						} else {
							$id1 = trim($_POST['iid1']);
						}
						if(empty(trim($_POST['iid2']))){
							$id2 = 'Empty';
						} else {
							$id2 = trim($_POST['iid2']);
						}
						if(empty(trim($_POST['iid3']))){
							$id3 = 'Empty';
						} else {
							$id3 = trim($_POST['iid3']);
						}
						if(empty(trim($_POST['iid4']))){
							$id4 = 'Empty';
						} else {
							$id4 = trim($_POST['iid4']);
						}
						if(empty(trim($_POST['iid5']))){
							$id5 = 'Empty';
						} else {
							$id5 = trim($_POST['iid5']);
						}
						if(empty(trim($_POST['iid6']))){
							$id6 = 'Empty';
						} else {
							$id6 = trim($_POST['iid6']);
						}
						if(empty(trim($_POST['iid7']))){
							$id7 = 'Empty';
						} else {
							$id7 = trim($_POST['iid7']);
						}
						if(empty(trim($_POST['iid8']))){
							$id8 = 'Empty';
						} else {
							$id8 = trim($_POST['iid8']);
						}
						if(empty(trim($_POST['iid9']))){
							if($id1 == 'Empty' && $id2 == 'Empty' && $id3 == 'Empty' && $id4 == 'Empty' && $id5 == 'Empty' && $id6 == 'Empty' && $id7 == 'Empty' && $id8 == 'Empty'){
								$errors[] = 'Recipe Input Cannot be Empty';
							} else {
								$id9 = 'Empty';
							}
						} else {
						...
						if(empty(trim($_POST['cr_oamnt']))){
							$oamnt = '1';
						} else {
							$oamnt = trim($_POST['cr_oamnt']);
						}
						if(empty(trim($_POST['cr_shplss']))){
							$shplss = '0';
						} else {
							$shplss = trim($_POST['cr_shplss']);
							if($shplss != '0' && $shplss != '1'){
								$errors[] = 'Is Shapeless Must be 0/1';
							}
						}
						if(($id1 == 'Empty' && $id2 == 'Empty' && $id3 == 'Empty' && $id4 == 'Empty' && $id7 == 'Empty') ||
							($id1 == 'Empty' && $id2 == 'Empty' && $id3 == 'Empty' && $id6 == 'Empty' && $id9 == 'Empty') ||
							($id1 == 'Empty' && $id4 == 'Empty' && $id7 == 'Empty' && $id8 == 'Empty' && $id9 == 'Empty') ||
							($id3 == 'Empty' && $id6 == 'Empty' && $id7 == 'Empty' && $id8 == 'Empty' && $id9 == 'Empty')){
							$tbt = '1';
						} else {
							$tbt = '0';
						}
						...
						if(empty($errors)){
							...
							$query = "INSERT INTO crafting_recipe (IID1, IID2, IID3, IID4, IID5, IID6, IID7, IID8, IID9, OID, OAmnt, Shplss, TBT)
										VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssssssssssiii", $id1, $id2, $id3, $id4, $id5, $id6, $id7, $id8, $id9, $id10, $oamnt, $shplss, $tbt);
