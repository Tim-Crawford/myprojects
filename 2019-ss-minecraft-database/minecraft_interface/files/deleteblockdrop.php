	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deleteblockdrop.php" method="post">
						<b>Delete Block Drop</b>
						<p>Item ID:
					...
					if(isset($_POST['submit'])){
						...
						if(empty(trim($_POST['bd_iid']))){
							...
							$id1 = trim($_POST['bd_iid']);
						}
						if(empty(trim($_POST['bd_bid']))){
							$errors[] = 'Missing Block ID';
						} else {
							$id2 = trim($_POST['bd_bid']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "DELETE FROM block_drop WHERE IID = ? AND BID = ?";
							...
							if($affected_rows == 1) {
								echo 'Block Drop Deleted Successfully';
