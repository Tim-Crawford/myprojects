	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertstatuseffect.php" method="post">
						<b>Insert Status Effect</b>
						<p>ID:
							<input type="text" name="se_id" size="30" value="" />
						</p>
						<p>Name:
							<input type="text" name="se_name" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['se_id']))){
							$errors[] = 'Missing Status Effect ID';
						} else {
							$id = trim($_POST['se_id']);
						}
						if(empty(trim($_POST['se_name']))){
							$errors[] = 'Missing Status Effect Name';
						} else {
							$name = trim($_POST['se_name']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "INSERT INTO status_effect (ID, Name)
										VALUES (?,?)";
							...
							if($affected_rows == 1) {
								echo 'Status Effect Inserted Successfully';
