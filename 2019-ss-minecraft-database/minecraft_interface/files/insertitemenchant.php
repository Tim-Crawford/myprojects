	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertitemenchant.php" method="post">
						<b>Insert Item Enchant</b>
						<p>Item ID:
							<input type="text" name="e_iid" size="30" value="" />
						</p>
						<p>Enchantment ID:
							<input type="text" name="e_enid" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['e_iid']))){
							$errors[] = 'Missing Item ID';
						} else {
							$iid = trim($_POST['e_iid']);
						}
						if(empty(trim($_POST['e_enid']))){
							$errors[] = 'Missing Enchantment ID';
						} else {
							$enid = trim($_POST['e_enid']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "INSERT INTO enchant (IID, EnID)
										VALUES (?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ss", $iid, $enid);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Item Enchant Inserted Successfully';
