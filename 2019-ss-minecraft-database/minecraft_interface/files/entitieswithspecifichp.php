	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/entitieswithspecifichp.php" method="post">
						<b>Entities with Specific HP</b>
						<p>HP:
							<input type="text" name="hp_hp" size="30" value="" />
						</p>
						<p>Greater/Equal/Less:
							<input type="text" name="hp_gel" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['hp_hp']))){
							$errors[] = 'Missing HP';
						} else {
							$hp = trim($_POST['hp_hp']);
						}
						if(empty(trim($_POST['hp_gel']))){
							$errors[] = 'Missing Greater/Equal/Less';
						} else {
							$gel = trim($_POST['hp_gel']);
						...
						if(empty($errors)){
							...
							if($gel == 'Greater'){
								$query = "SELECT Name, HP FROM entity WHERE HP >  '" . $hp . "' ORDER BY Name ASC";
								...
								$query = "SELECT Name, HP FROM entity WHERE HP > 0 AND HP <  '" . $hp . "' ORDER BY Name ASC";
								$response = @mysqli_query($dbc, $query);
