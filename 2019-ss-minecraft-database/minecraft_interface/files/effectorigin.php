	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/effectorigin.php" method="post">
						<b>Effect Origin</b>
						<p>Status Effect Name:
							<input type="text" name="sename" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['sename']))){
							$errors[] = 'Missing Status Effect Name';
						} else {
							$name = trim($_POST['sename']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "SELECT I.Name, E.Level, E.Durat, E.Cause FROM item_effect E, item I, status_effect S WHERE S.Name = '" . $name . "' AND S.ID = E.SEID AND E.IID = I.ID ORDER BY I.Name, E.Level, E.Durat ASC";
							...
							if($response){
								echo '</td><td valign="top"><table align="left" cellspacing="5" cellpadding="8">
										<tr>
											...
											<td align="left"><b>Cause</b></td>
										</tr>';
								while($row = mysqli_fetch_array($response)){
									echo '<tr><td align=align"left">' .
									$row['Name'] . '</td><td align="left">' .
									$row['Level'] . '</td><td align="left">' .
									$row['Durat']/20 . ' s</td><td align="left">' .
									$row['Cause'] . '</td><td align="left"></tr>';
								}
							...
							$query = "SELECT B.Name, E.Level, E.Durat, E.Cause FROM block_effect E, block B, status_effect S WHERE S.Name = '" . $name . "' AND S.ID = E.SEID AND E.BID = B.ID ORDER BY B.Name, E.Level, E.Durat ASC";
							$response = @mysqli_query($dbc, $query);
							...
							$query = "SELECT En.Name, E.Level, E.Durat, E.Cause FROM entity_effect E, entity En, status_effect S WHERE S.Name = '" . $name . "' AND S.ID = E.SEID AND E.EID = En.ID ORDER BY En.Name, E.Level, E.Durat ASC";
							$response = @mysqli_query($dbc, $query);
							
							if($response){
								echo '</td><td valign="top"><table align="left" cellspacing="5" cellpadding="8">
										<tr>
											<td align="left"><b>Entity</b></td>
											<td align="left"><b>Level</b></td>
											<td align="left"><b>Duration</b></td>
											<td align="left"><b>Cause</b></td>
										</tr>';
