	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertitemeffect.php" method="post">
						<b>Insert Item Effect</b>
						<p>Status Effect ID:
							<input type="text" name="ie_seid" size="30" value="" />
						</p>
						<p>Item ID:
							<input type="text" name="ie_iid" size="30" value="" />
						</p>
						<p>Level:
							<input type="text" name="ie_level" size="30" value="" />
						</p>
						<p>Cause:
							<input type="text" name="ie_cause" size="30" value="" />
						</p>
						<p>Duration in Ticks:
							<input type="text" name="ie_durat" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['ie_seid']))){
							$errors[] = 'Missing Status Effect ID';
						} else {
							$seid = trim($_POST['ie_seid']);
						}
						if(empty(trim($_POST['ie_iid']))){
							$errors[] = 'Missing Item ID';
						} else {
							$iid = trim($_POST['ie_iid']);
						}
						if(empty(trim($_POST['ie_level']))){
							$level = '1';
						} else {
							$level = trim($_POST['ie_level']);
							if($level <= '0'){
								$errors[] = 'Level Must be Greater than 0';
							}
						}
						if(empty(trim($_POST['ie_cause']))){
							$errors[] = 'Missing Cause';
						} else {
							$cause = trim($_POST['ie_cause']);
						}
						if(empty(trim($_POST['ie_durat']))){
							$durat = '1';
						} else {
							$durat = trim($_POST['ie_durat']);
							if($durat <= '0'){
						...
						if(empty($errors)){
							...
							$query = "INSERT INTO item_effect (IID, SEID, Level, Cause, Durat)
										VALUES (?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssisi", $iid, $seid, $level, $cause, $durat);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Item Effect Inserted Successfully';
