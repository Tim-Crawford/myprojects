	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertbrewingrecipe.php" method="post">
						<b>Insert Brewing Recipe</b>
						<p>Potion ID:
							<input type="text" name="pid" size="30" value="" />
						</p>
						<p>Ingredient ID:
							<input type="text" name="inid" size="30" value="" />
						</p>
						<p>Output ID:
							<input type="text" name="oid" size="30" value="" />
						</p>
						<p>Input Potion Name:
							<input type="text" name="br_iname" size="30" value="" />
						</p>
						<p>Output Potion Name:
							<input type="text" name="br_oname" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						...
						if(empty(trim($_POST['br_iname']))){
							$errors[] = 'Missing Input Name';
						} else {
							$iname = trim($_POST['br_iname']);
						}
						if(empty(trim($_POST['br_oname']))){
							$errors[] = 'Missing Output Name';
						} else {
							$oname = trim($_POST['br_oname']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "INSERT INTO brewing_recipe (PID, InID, OID, IName, OName)
										VALUES (?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "sssss", $id1, $id2, $id3, $iname, $oname);
