	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deleteentityeffect.php" method="post">
						<b>Delete Entity Effect</b>
						<p>Entity ID:
					...
					if(isset($_POST['submit'])){
						...
						if(empty($errors)){
							...
							$query = "DELETE FROM entity_effect WHERE EID = ? AND SEID = ? AND Level = ? AND Durat = ?";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssii", $id1, $id2, $level, $durat);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Entity Effect Deleted Successfully';
