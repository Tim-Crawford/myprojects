	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertentityeffect.php" method="post">
						<b>Insert Entity Effect</b>
						<p>Status Effect ID:
							<input type="text" name="ee_seid" size="30" value="" />
						</p>
						<p>Entity ID:
							<input type="text" name="ee_eid" size="30" value="" />
						</p>
						<p>Level:
							<input type="text" name="ee_level" size="30" value="" />
						</p>
						<p>Cause:
							<input type="text" name="ee_cause" size="30" value="" />
						</p>
						<p>Duration in Ticks:
							<input type="text" name="ee_durat" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['ee_seid']))){
							$errors[] = 'Missing Status Effect ID';
						} else {
							$seid = trim($_POST['ee_seid']);
						}
						if(empty(trim($_POST['ee_eid']))){
							$errors[] = 'Missing Entity ID';
						} else {
							$eid = trim($_POST['ee_eid']);
						}
						if(empty(trim($_POST['ee_level']))){
							$level = '1';
						} else {
							$level = trim($_POST['ee_level']);
							if($level <= '0'){
								$errors[] = 'Level Must be Greater than 0';
							}
						}
						if(empty(trim($_POST['ee_cause']))){
							$errors[] = 'Missing Cause';
						} else {
							$cause = trim($_POST['ee_cause']);
						}
						if(empty(trim($_POST['ee_durat']))){
							$durat = '1';
						} else {
							$durat = trim($_POST['ee_durat']);
							if($durat <= '0'){
						...
						if(empty($errors)){
							...
							$query = "INSERT INTO entity_effect (EID, SEID, Level, Cause, Durat)
										VALUES (?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssisi", $eid, $seid, $level, $cause, $durat);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Entity Effect Inserted Successfully';
