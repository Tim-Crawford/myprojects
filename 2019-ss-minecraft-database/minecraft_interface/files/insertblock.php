	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertblock.php" method="post">
						<b>Insert Block</b>
						<p>ID:
							<input type="text" name="b_id" size="30" value="" />
						</p>
						<p>Name:
							<input type="text" name="b_name" size="30" value="" />
						</p>
						<p>Luminance:
							<input type="text" name="b_lum" size="30" value="" />
						</p>
						<p>Blast Resistance:
							<input type="text" name="b_br" size="30" value="" />
						</p>
						<p>Hardness:
							<input type="text" name="b_hard" size="30" value="" />
						</p>
						<p>Is Flammable:
							<input type="text" name="b_flam" size="30" value="" />
						</p>
						<p>Is Tile Entity:
							<input type="text" name="b_te" size="30" value="" />
						</p>
						<p>Is Inventory:
							<input type="text" name="b_invflg" size="30" value="" />
						</p>
						<p>Number of Slots:
							<input type="text" name="b_nslots" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['b_id']))){
							$errors[] = 'Missing Block ID';
						} else {
							$id = trim($_POST['b_id']);
						}
						if(empty(trim($_POST['b_name']))){
							$errors[] = 'Missing Block Name';
						} else {
							$name = trim($_POST['b_name']);
						}
						if(empty(trim($_POST['b_lum']))){
							$lum = '0';
						} else {
							$lum = trim($_POST['b_lum']);
							if($lum < '0' || $lum > '15'){
								$errors[] = 'Luminance Must be 0-15';
							}
						}
						if(empty(trim($_POST['b_br']))){
							$br = '0';
						} else {
							$br = trim($_POST['b_br']);
						}
						if(empty(trim($_POST['b_hard']))){
							$hard = '0';
						} else {
							$hard = trim($_POST['b_hard']);
						}
						if(empty(trim($_POST['b_flam']))){
							$flam = '0';
						} else {
							$flam = trim($_POST['b_flam']);
							if($flam != '0' && $flam != '1'){
								$errors[] = 'Is Flammable Must be 0/1';
							}
						}
						if(empty(trim($_POST['b_te']))){
							$te = '0';
						} else {
							$te = trim($_POST['b_te']);
							if($te != '0' && $te != '1'){
								$errors[] = 'Is Tile Entity Must be 0/1';
							}
						}
						if(empty(trim($_POST['b_invflg']))){
							$invflg = '0';
						} else {
							$invflg = trim($_POST['b_invflg']);
							if($invflg != '0' && $invflg != '1'){
								$errors[] = 'Is Inventory Must be 0/1';
							}
						}
						if($invflg == '0'){
							$nslots = NULL;
							...
							if(empty(trim($_POST['b_invflg']))){
								...
								$nslots = trim($_POST['b_nslots']);
								if($nslots <= '0'){
									$errors[] = 'Number of Slots Must be Greater Than 0';
								}
							}
						}
						...
						if(empty($errors)){
							...
							$query = "INSERT INTO block (ID, Name, Lum, BR, Hard, Flam, TE, InventoryFlag, NSlots)
										VALUES (?,?,?,?,?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssiddiiii", $id, $name, $lum, $br, $hard, $flam, $te, $invflg, $nslots);
