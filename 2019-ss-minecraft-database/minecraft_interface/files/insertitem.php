	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertitem.php" method="post">
						<b>Insert Item</b>
						<p>ID:
							<input type="text" name="i_id" size="30" value="" />
						</p>
						<p>Name:
							<input type="text" name="i_name" size="30" value="" />
						</p>
						<p>Stack Size:
							<input type="text" name="i_stksz" size="30" value="" />
						</p>
						<p>Durability:
							<input type="text" name="i_durab" size="30" value="" />
						</p>
						<p>Is Renewable:
							<input type="text" name="i_renew" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['i_id']))){
							$errors[] = 'Missing Item ID';
						} else {
							$id = trim($_POST['i_id']);
						}
						if(empty(trim($_POST['i_name']))){
							$errors[] = 'Missing Item Name';
						} else {
							$name = trim($_POST['i_name']);
						}
						if(empty(trim($_POST['i_stksz']))){
							$stksz = '1';
						} else {
							$stksz = trim($_POST['i_stksz']);
						}
						if(empty(trim($_POST['i_durab']))){
							$durab = '0';
						} else {
							$durab = trim($_POST['i_durab']);
						}
						if(empty(trim($_POST['i_renew']))){
							$renew = '0';
						} else {
							$renew = trim($_POST['i_renew']);
							if($renew != '0' && $renew != '1'){
								$errors[] = 'Is Renewable Must be 0/1';
							}
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "INSERT INTO item (ID, Name, StkSz, Durab, Renew)
										VALUES (?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssiii", $id, $name, $stksz, $durab, $renew);
