	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deletebrewingrecipe.php" method="post">
						<b>Delete Brewing Recipe</b>
						<p>Potion ID:
							<input type="text" name="br_pid" size="30" value="" />
						</p>
						<p>Ingredient ID:
							<input type="text" name="br_iid" size="30" value="" />
						</p>
						<p>Output ID:
							<input type="text" name="br_oid" size="30" value="" />
						</p>
						<p>Input Potion Name:
							<input type="text" name="br_ipn" size="30" value="" />
						</p>
						<p>Output Potion Name:
							<input type="text" name="br_opn" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['br_pid']))){
							$errors[] = 'Missing Potion ID';
						} else {
							$id1 = trim($_POST['br_pid']);
						}
						if(empty(trim($_POST['br_iid']))){
							$errors[] = 'Missing Ingredient ID';
						} else {
							$id2 = trim($_POST['br_iid']);
						}
						if(empty(trim($_POST['br_oid']))){
							$errors[] = 'Missing Output ID';
						} else {
							$id3 = trim($_POST['br_oid']);
						}
						if(empty(trim($_POST['br_ipn']))){
							$errors[] = 'Missing Input Potion Name';
						} else {
							$in = trim($_POST['br_ipn']);
						}
						if(empty(trim($_POST['br_opn']))){
							$errors[] = 'Missing Output Potion Name';
						} else {
							$on = trim($_POST['br_opn']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "DELETE FROM brewing_recipe WHERE PID = ? AND InID = ? AND OID = ? AND IName = ? AND OName = ?";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "sssss", $id1, $id2, $id3, $in, $on);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Brewing Recipe Deleted Successfully';
