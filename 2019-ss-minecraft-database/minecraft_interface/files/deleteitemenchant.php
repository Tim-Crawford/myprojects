	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/deleteitemenchant.php" method="post">
						<b>Delete Item Enchant</b>
						<p>Item ID:
							<input type="text" name="ie_iid" size="30" value="" />
						</p>
						<p>Enchantment ID:
							<input type="text" name="ie_eid" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						...
						if(empty(trim($_POST['ie_eid']))){
							$errors[] = 'Missing Enchantment ID';
						} else {
							$id2 = trim($_POST['ie_eid']);
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "DELETE FROM enchant WHERE IID = ? AND EnID = ?";
							...
							if($affected_rows == 1) {
								echo 'Item Enchant Deleted Successfully';
