	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/itemswithspecificdurability.php" method="post">
						<b>Items with Specific Durability</b>
						<p>Durability:
							<input type="text" name="d_durab" size="30" value="" />
						</p>
						<p>Greater/Equal/Less:
							<input type="text" name="d_gel" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['d_durab']))){
							$errors[] = 'Missing Durability';
						} else {
							$durab = trim($_POST['d_durab']);
						}
						if(empty(trim($_POST['d_gel']))){
							$errors[] = 'Missing Greater/Equal/Less';
						} else {
							$gel = trim($_POST['d_gel']);
						...
						if(empty($errors)){
							...
							if($gel == 'Greater'){
								$query = "SELECT Name, Durab FROM item WHERE Durab >  '" . $durab . "' ORDER BY Name ASC";
								...
								if($response){
									echo '</td><td valign="top"><table align="left" cellspacing="5" cellpadding="8">
											<tr>
												...
												<td align="left"><b>Durability</b></td>
											</tr>';
									while($row = mysqli_fetch_array($response)){
										echo '<tr><td align=align"left">' .
										$row['Name'] . '</td><td align="left">' .
										$row['Durab'] . '</td><td align="left"></tr>';
									}
								...
								$query = "SELECT Name, Durab FROM item WHERE Durab > 0 AND Durab <  '" . $durab . "' ORDER BY Name ASC";
								$response = @mysqli_query($dbc, $query);
