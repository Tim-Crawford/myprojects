	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertblockeffect.php" method="post">
						<b>Insert Block Effect</b>
						<p>Status Effect ID:
							<input type="text" name="be_seid" size="30" value="" />
						</p>
						<p>Block ID:
							<input type="text" name="be_bid" size="30" value="" />
						</p>
						<p>Level:
							<input type="text" name="be_level" size="30" value="" />
						</p>
						<p>Cause:
							<input type="text" name="be_cause" size="30" value="" />
						</p>
						<p>Duration in Ticks:
							<input type="text" name="be_durat" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['be_seid']))){
							$errors[] = 'Missing Status Effect ID';
						} else {
							$seid = trim($_POST['be_seid']);
						}
						if(empty(trim($_POST['be_bid']))){
							$errors[] = 'Missing Block ID';
						} else {
							$bid = trim($_POST['be_bid']);
						}
						if(empty(trim($_POST['be_level']))){
							$level = '1';
						} else {
							$level = trim($_POST['be_level']);
							if($level <= '0'){
								$errors[] = 'Level Must be Greater than 0';
							}
						}
						if(empty(trim($_POST['be_cause']))){
							$errors[] = 'Missing Cause';
						} else {
							$cause = trim($_POST['be_cause']);
						}
						if(empty(trim($_POST['be_durat']))){
							$durat = '1';
						} else {
							$durat = trim($_POST['be_durat']);
							if($durat <= '0'){
						...
						if(empty($errors)){
							...
							$query = "INSERT INTO block_effect (BID, SEID, Level, Cause, Durat)
										VALUES (?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssisi", $bid, $seid, $level, $cause, $durat);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Block Effect Inserted Successfully';
