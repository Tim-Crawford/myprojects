	...
	<body>
		...
		<table cellpadding="20">
			...
			<tr>
				...
				<td valign="top">
					<form action="http://localhost/minecraft_interface/files/insertentitydrop.php" method="post">
						<b>Insert Entity Drop</b>
						<p>Item ID:
							<input type="text" name="ed_iid" size="30" value="" />
						</p>
						<p>Entity ID:
							<input type="text" name="ed_eid" size="30" value="" />
						</p>
						<p>Is Drop Affected by Looting:
							<input type="text" name="ed_lt" size="30" value="" />
						</p>
						<p>Minimum Drop Amount:
							<input type="text" name="ed_min" size="30" value="" />
						</p>
						<p>Maximum Drop Amount:
							<input type="text" name="ed_max" size="30" value="" />
						</p>
						<p>Drop Chance:
							<input type="text" name="ed_c" size="30" value="" />
						</p>
						<p>
					...
					if(isset($_POST['submit'])){
						$errors = array();
						if(empty(trim($_POST['ed_iid']))){
							$errors[] = 'Missing Item ID';
						} else {
							$iid = trim($_POST['ed_iid']);
						}
						if(empty(trim($_POST['ed_eid']))){
							$errors[] = 'Missing Entity ID';
						} else {
							$eid = trim($_POST['ed_eid']);
						}
						if(empty(trim($_POST['ed_lt']))){
							$lt = '0';
						} else {
							$lt = trim($_POST['ed_lt']);
							if($lt != '0' && $lt != '1'){
								$errors[] = 'Is Drop Affected by Looting Must be 0/1';
							}
						}
						if(empty(trim($_POST['ed_min']))){
							$min = '0';
						} else {
							$min = trim($_POST['ed_min']);
							if($min < '0'){
								$errors[] = 'Min Drop Amount Cannot be Negative';
							}
						}
						if(empty(trim($_POST['ed_max']))){
							$max = '1';
						} else {
							$max = trim($_POST['ed_max']);
							if($max <= '0'){
						...
						if(empty(trim($_POST['ed_c']))){
							$chance = '100';
						} else {
							$chance = trim($_POST['ed_c']);
							if($chance < '0' || $chance > '100'){
								$errors[] = 'Drop Chance Must be 0-100';
							}
						}
						
						if(empty($errors)){
							require_once('../mysqli_connect.php');
							$query = "INSERT INTO entity_drop (IID, EID, Lt, MinAmnt, MaxAmnt, Chance)
										VALUES (?,?,?,?,?,?)";
							$stmt = mysqli_prepare($dbc, $query);
							
							mysqli_stmt_bind_param($stmt, "ssiiid", $iid, $eid, $lt, $min, $max, $chance);
							mysqli_stmt_execute($stmt);
							$affected_rows = mysqli_stmt_affected_rows($stmt);
							if($affected_rows == 1) {
								echo 'Entity Drop Inserted Successfully';
